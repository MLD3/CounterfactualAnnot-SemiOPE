...
parser.add_argument('--annot_noise', type=float, default=0.2)
parser.add_argument('--annot_ratio', type=float, default=1.0)
args = parser.parse_args()
pol_flip_num = args.flip_num
pol_flip_seed = args.flip_seed
annot_noise = args.annot_noise
annot_ratio = args.annot_ratio

pol_name = f'flip{pol_flip_num}_seed{pol_flip_seed}'
out_fname = f'./results/{exp_name}/vaso_eps_{eps_str}-{pol_name}-aug_step-annotEval-Noise_{annot_noise}-MissingImpute_{annot_ratio}.csv'
...
rng_cf = np.random.default_rng(42424242)
...
for run in range(runs):
    ...
    # only draw cf annot with probability pcf
    for i, row in tqdm(list(df_va.iterrows()), disable=True):
        map_pt_id = row['map_pt_id']
        h = int(row['Time'])
        s = int(row['State'])
        if row['NextState'] in [1442]:
            a_cf = int(row['Action'])
            if rng_cf.uniform() < annot_ratio:
                df_va.loc[i, 'Reward'] = Q_H_eval[h][s, a_cf] + rng_annot.normal(0, annot_noise)
            else:
                df_va.loc[i, 'Reward'] = np.nan

    # impute the missing annots
    for s, a in itertools.product(range(nS), range(nA)):
        sa_cf_mask = (df_va['State'] == s) & (df_va['Action'] == a) & (df_va['NextState'] == 1442)
        if not df_va.loc[sa_cf_mask, 'Reward'].notnull().any():
            continue
        avg_annot = df_va.loc[sa_cf_mask, 'Reward'].dropna().mean()
        df_va.loc[sa_cf_mask, 'Reward'] = df_va.loc[sa_cf_mask, 'Reward'].fillna(avg_annot)

    # only use the cf annot with probability pcf
    keep_mask = []
    for i, row in tqdm(list(df_va.iterrows()), disable=True):
        map_pt_id = row['map_pt_id']
        h = int(row['Time'])
        ...
        if row['NextState'] in [1440, 1441]:
            ...
            if not np.isnan(row['Reward']):
                keep_mask.append(True)
            else:
                keep_mask.append(False)
        else:
            keep_mask.append(True)
    df_va = df_va.loc[keep_mask]
    ...
    for i, row in tqdm(list(df_va.iterrows()), disable=True):
        ...
        if row['NextState'] in [1440, 1441]:
            ...
            pass
    df_va['Weight'] = df_va['Weight'].fillna(1.0)
