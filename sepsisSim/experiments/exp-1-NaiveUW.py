...
out_fname = f'./results/{exp_name}/vaso_eps_{eps_str}-{pol_name}-aug_step-NaiveUW.csv'
...
from OPE_utils import compute_behavior_policy
...
def compute_behavior_policy_weighted(df_data, trajW=None):
    """
    Calculate probabilities of the behavior policy π_b
    using Maximum Likelihood Estimation (MLE)
    """
    if trajW is None:
        trajW = np.eye(len(df_data))
...
#     assert np.isclose(traj_weight_matrix.sum(axis=0), 1).all()
    ...
    v2_pi_b_val = compute_behavior_policy(df_va)

    # OPE - WIS
    v2_IS_value, v2_WIS_value, v2_ESS_info = OPE_IS_trajW(v2_data_va, v2_pi_b_val, π_eval, gamma)
